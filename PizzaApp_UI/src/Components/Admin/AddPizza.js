...
const AddPizza = () => {
  ...
  const addPizza = async () => {
    ...
    try {
      const result = await AdminPizzaServices.addPizza(name, price, category, ingredients, pizaImage);
      if (result) {
        navigate('/pizza-list');
      }
    } catch (error) {
  ...
  return (
    ...
    <div className="text-center ">
      <button type="button" onClick={addPizza} className="btn btn-primary appButton text-dark">Add Pizza</button>
    </div>
  </div>
